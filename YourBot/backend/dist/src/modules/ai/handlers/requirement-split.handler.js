...
exports.RequirementSplitHandler = void 0;
const common_1 = require("@nestjs/common");
const output_validator_1 = require("../output-validator");
const output_validator_2 = require("../output-validator");
...
function buildPrompt(input) {
    const versionConfig = config_1.PROMPT_VERSIONS[enums_1.AiJobType.REQUIREMENT_SPLIT];
    ...
    prompt = prompt.replace('{{deliverables}}', Array.isArray(input.deliverables)
        ? input.deliverables
...
class RequirementSplitHandler {
    aiClient;
    logger = new common_1.Logger(RequirementSplitHandler.name);
    constructor(aiClient) {
        this.aiClient = aiClient;
    }
    async execute(inputPayload) {
        const input = inputPayload;
        this.logger.log(`Processing requirement split for requirement_id=${input.requirement_id}`);
        ...
        let validatedOutput;
        try {
            validatedOutput = (0, output_validator_1.validateAiOutput)(rawResponse, output_validator_2.TaskDecompositionOutputSchema);
        }
        ...
        this.logger.log(`Validated task decomposition output: ${validatedOutput.tasks.length} tasks`);
        return {
...
exports.RequirementSplitHandler = RequirementSplitHandler;
//# sourceMappingURL=requirement-split.handler.js.map